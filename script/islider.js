 ...
 */
var iSlider, islider_core, plugins_islider_animate, plugins_islider_dot;
iSlider = function () {
  ...
  iSlider.prototype._renderHTML = function () {
    ...
    var len = ((this.data.length>1)?3:2);
    var leni = ((this.data.length>1)?0:1);

    if(this.animateType != 'card'){
        len = 3;
        leni = 0;
    }

    for (var i = leni; i < len; i++) {
  ...
  */
  ...
  iSlider.prototype.handleEvent = function (evt) {
    if(this.data.length==1 && this.animateType == 'card'){evt.preventDefault();return false;}
    var device = this._device();
    switch (evt.type) {
  ...
  var extendAnimation = {
    'depth': function (dom, axis, scale, i, offset) {
      ...
      dom.style.webkitTransform = 'scale(' + zoomScale + ', ' + zoomScale + ') translateZ(0) translate'
          + axis + '(' + (offset +(i-1)*80*zoomScale+ scale * (i - 1)) + 'px)';
    },
    'touch': function (dom, axis, scale, i, offset) {
         var zoomScale = 1;
         this.wrap.style.webkitPerspective = scale * 4;
         zoomScale=1;//(i===1) ? 1 :0.8;
         dom.style.zIndex = (i === 1) ? 100 : (offset > 0) ? (1 - i) : (i - 1);
         dom.style.webkitTransform = 'scale(' + zoomScale + ', ' + zoomScale + ') translateZ(0) translate'
              + axis + '(' + (offset +(i-1)*70*zoomScale+ scale * (i - 1)) + 'px)';

        $$(dom).removeAttr('data-link');
        if((dom.style.zIndex == 100)&& (offset == 0)){
            $$(dom).attr('data-link',true);
            setTimeout(function(){
                if($$(".page.page-on-center").length>0){
                    var wp = document.querySelector(".view-main .pages .page.page-on-center").getAttribute('data-page');
                    var obj = document.querySelector('.page.'+ wp +' .page-content-inner');
                    var cl = $$(obj).data('list');
                    obj = document.querySelector('.page.'+ wp +' .page-content-inner .'+cl);
                    var ob = document.querySelector('.page.'+ wp +' .islider-dom[data-link] .animate');
                    $$(obj).val($$(ob).data('id')).trigger('change');
                }
            },800);
        }
    },
    'touch_list': function (dom, axis, scale, i, offset) {
          var zoomScale = 1;
          this.wrap.style.webkitPerspective = scale * 4;
          zoomScale= (i===1) ? 1 :0.9;
          dom.style.zIndex = (i === 1) ? 100 : (offset > 0) ? (1 - i) : (i - 1);
      ...
      'card': function (dom, axis, scale, i, offset) {
          ...
          dom.style.boxShadow = '0px 5px 40px #fff';

          var translateY = (offset +(i-1)*100);
          if(dom.style.zIndex == -1){
              dom.style.opacity = 0.5;
              translateY = (offset +1.5*100);
          }else if(dom.style.zIndex == 100){
              dom.style.opacity = 1;
              dom.style.boxShadow = '#ef9356 0px 5px 30px';
          }else if(dom.style.zIndex == 1){
              dom.style.opacity = 0.8;
              dom.style.width = '110%';
              dom.style.marginLeft = "-5%";
          }
    ...
    'cards': function (dom, axis, scale, i, offset) {
        var zoomScale = 1;
        this.wrap.style.webkitPerspective = scale * 4;
        zoomScale=(i===1) ? 1 :0.8;
        return;
        if((dom.style.zIndex == 100)&& (offset == 0)){
            dom.style.marginTop = '-200%';
            dom.style.zIndex = -1;
            setTimeout(function(){
                var wp = document.querySelector(".view-main .pages .page.page-on-center").getAttribute('data-page');
                var objs = document.querySelectorAll('.page.'+ wp +' .islider-dom');
                for(var i in objs){
                    var obj = objs[i];
                    if(obj.style.zIndex && obj.style.zIndex == -1){
                        ...
                        obj.style.boxShadow = '0px 5px 40px #fff';
                        obj.style.webkitTransform = 'scale(0.8, 0.8) translateZ(0) translateY(150px)';
                        break;
                    }
                }
            },500);
